...
import pandas as pd
import matplotlib
matplotlib.use('Agg') # Mandatory for 500MB RAM
...
# --- DYNAMIC PATH LOGIC ---
# This script is in src/simulation/scripts/
# We need to reach src/logger/data/ and src/simulation/output/
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(SCRIPT_DIR)))

DATA_FILE = os.path.join(PROJECT_ROOT, "src", "logger", "data", "2_inflight", "MPU6050.txt")
OUTPUT_DIR = os.path.join(PROJECT_ROOT, "src", "simulation", "output")
# Match the filename exactly as expected by app_server.py
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "mission_sim.mp4")
...
try:
    if not os.path.exists(DATA_FILE):
        print(f"Data file missing at: {DATA_FILE}")
        sys.exit(1)
...
# Using float32 to keep data size small (Vital for 500MB RAM)
...
ax_tele.set_title("Z-ACCEL TELEMETRY", fontsize=8, color='gray')
...
print(f"Starting memory-locked render to: {OUTPUT_FILE}")
try:
    # Forced low bitrate and low DPI to ensure success on 500MB RAM
    ani.save(OUTPUT_FILE, fps=15, dpi=RENDER_DPI, writer='ffmpeg', bitrate=350)
