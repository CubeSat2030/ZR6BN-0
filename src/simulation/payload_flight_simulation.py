...
import os, sys, math, time, shutil, subprocess, warnings
import numpy as np
import pandas as pd
import matplotlib
...
def shade_faces(rotmat, faces_idx, verts, base=(0.78,0.72,1.0)):
    ...
    for f in faces_idx:
        ...
        facecols.append(tuple(np.clip(np.array(base)*(0.45 + 0.55*lam), 0, 1)))
    return rv, facecols
...
left_ax.axis("off")
left_ax.set_xlim([-1.0, 1.0])
left_ax.set_ylim([-1.0, 1.0])
left_ax.set_zlim([-1.6, 1.2])
...
for i in tqdm(range(nframes), desc="Render frames", unit="frame"):
    ...
    offset = np.array([float(traj_x[i]), float(traj_y[i]), float(traj_z[i])])
    rv_t = rv + offset
    ...
    if alt[i] < 12000:
        ...
        halo = left_ax.scatter([offset[0]], [offset[1]], [offset[2]-0.02], s=900*glow_strength,
...
# ---------------- ENCODE ----------------
print("Encoding MP4 with ffmpeg (GPU if available)...")
nvenc_cmd = (
...
if ret != 0:
    ...
    if ret != 0:
        raise RuntimeError(f"ffmpeg encoding failed (return {ret}).")
