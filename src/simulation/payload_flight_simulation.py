...
# read with flexible parsing; skip comments starting '#' (your file already used that)
...
# ------------------------------------------------------------------
try:
    from mpu6050_plotter import create_mpu6050_chart
    _HAS_PLOTTER = True
except Exception:
    _HAS_PLOTTER = False
...
if not _PLOTTER_USED:
    ...
    # Gyro (x,y,z)
    ax_gyro.plot(df["timestamp"], df["gyro_x_dps"], label="gx", lw=0.7)
    ax_gyro.plot(df["timestamp"], df["gyro_y_dps"], label="gy", lw=0.7)
    ax_gyro.plot(df["timestamp"], df["gyro_z_dps"], label="gz", lw=0.7)
